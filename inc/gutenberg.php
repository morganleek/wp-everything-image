<?php
		...
		if( $block ) {
			if( !empty( $block['blockName'] ) ) {
				...
				if( $block['blockName'] == 'core/image' && isset( $block['attrs']['id'] ) ) {
					$sizes = wei_block_image_sizes( $block['attrs']['id'], $_args['path'] );
					$image = wei_image( // Image as Picture tag
						$block['attrs']['id'], 
						array(
							'type' => 'image',
							'sizes' => $sizes,
							'class' => 'wp-block-everything-image',
							'return' => true
						)
					);
					$block['innerHTML'] = $image;
					$block['innerContent'][0] = $image;
					// ___( implode( '|', $_args['path'] ) );
				}
				// ___( $block['innerBlocks'] );
	...
	function wei_block_image_sizes( $id = 0, $path = 0 ) {
		...
		// sizes filter
		$resize_array = apply_filters( 'wei_add_gutenberg_size', $resize_array );
		
		foreach( $resize_array as $k => $r ) {
			if( in_array( $k, $path ) ) {
				return $resize_array[$k];
			}
		}
		return $default;
	}
